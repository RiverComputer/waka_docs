...
On a technical level, Waka is an assembly of open protocol primitives for turning anchored evidence into portable, witnessed claims. Used as a sovereign data trust, Waka anchors data across a range of persistent and decentralized storage substrates. Using the claims engine, that data is verified into claims through Waka's composable witness layer, where human and machine validation methods can be composed and applied to data in accordance with a project's unique witness policy. The resulting claims can feed into a diverse set of issuance systems. They may support certificates, ecological credits, on-chain triggers, program evaluations, insurance claims, programmable smart contract remittance, or other instruments built on verified evidence.

At its root, Waka uses Decentralized Identifiers (DIDs), the Verifiable Credentials (VC) data model, AT Protocol's Personal Data Server with `did:plc` and CGS, Common Expression Language, and custom sidecars to the Hypercerts v2 lexicon space.

Identity in front and anchoring behind are both held to maximum versatility: any wallet or DID method in; any number of durable, on-chain, or ecological substrates out. The witness layer in the middle is therefore never bound to a single stack. By design, Waka does not require lock-in to a particular ecosystem or stack — rather, it composes with the ones you are already familiar with.
