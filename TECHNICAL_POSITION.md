...
On a technical level Waka is a composable witness layer — a thin assembly of open web primitives that does the one thing those primitives do not do on their own: turn a body of raw evidence into a portable, witnessed claim. The primitives are Decentralized Identifiers, the Verifiable Credentials data model, AT Protocol's Personal Data Server with `did:plc` and CGS, Common Expression Language for deterministic machine evaluation, and the Hypercerts v2 lexicon space. The mechanism is a claims engine in which composable witness methods and policies become a designable form of validation criteria for the creation of claims — human and machine witness composed, per context, under a signed policy — with the identity layer in front of that engine and the anchoring layer behind it both held to maximum versatility: any wallet and any DID method in, any of several durable, on-chain, and ecological substrates out, so the engine in the middle is never bound to a single stack. It re-derives no distributed protocol and hosts no account; what it is, end to end, is a small set of records, a witness engine, and the discipline of building only at the layer of the web that no company owns.
